...
<template>
  <div
    v-if="visible"
    class="fixed bottom-4 right-4 z-50 max-h-[calc(100vh-2rem)] w-[calc(100vw-2rem)] max-w-xs overflow-y-auto border-2 border-[#1cff2f] bg-black p-3 text-white shadow-[5px_5px_0_#1cff2f] sm:w-72 md:bottom-6 md:right-6"
  >
    <button
      type="button"
      aria-label="Cerrar popup"
      class="absolute right-2 top-2 flex size-7 items-center justify-center border-2 border-[#1cff2f] bg-black text-lg font-bold leading-none text-[#1cff2f] transition-all duration-300 ease-out  hover:border-[#1cff2f] hover:bg-[#1cff2f] hover:text-black "
      @click="visible = false"
    >
      &times;
    </button>

    <p class="pr-8 text-[0.65rem] font-bold uppercase tracking-[0.22em] text-[#1cff2f]">
      Las entradas suben de precio en
    </p>

    <div class="mt-4 grid grid-cols-4 gap-1.5">
      <div class="border-2 border-[#1cff2f] bg-black px-1.5 py-2 text-center ">
      ...
      <div class="border-2 border-[#1cff2f] bg-black px-1.5 py-2 text-center ">
        <strong class="block text-2xl font-black leading-none text-[#1cff2f]">{{ seconds }}</strong>
    ...
    <a
      href="#tickets"
      class="mt-4 inline-flex w-full items-center justify-center border-2 border-[#1cff2f] bg-[#1cff2f] px-4 py-2 text-xs font-black uppercase tracking-[0.16em] text-black transition-all duration-300 ease-out hover:-translate-y-1 hover:bg-black hover:text-[#1cff2f]"
    >
      Comprar ahora
    </a>
  </div>
